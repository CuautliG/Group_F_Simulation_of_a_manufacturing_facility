...
float clockfcn(float input_1, float input_2, float input_3, float input_4, float input_5){
    ...
    if ((input_1<=0)&&(input_2<=0)&&(input_3<=0)&&(input_4<=0)&&(input_5<=0)){ /*precondition test - every input must positive)*/
        puts("Error in clockfcn: all inputs are non-positive");
        puts("->The function expected that at least one of the input was positive");
        return 0;
    }
    if (input_1 >0){
...
if (((input_1>0)&&(internal_process_time > input_1)) || ((input_2>0)&&(internal_process_time > input_2)) || ((input_3>0)&&(internal_process_time > input_3)) || ((input_4>0)&&(internal_process_time > input_4)) || ((input_5>0)&&(internal_process_time > input_5)) )
    {
        puts("Error in clockfcn: the output is not the minimum positive number");
        puts("->The function expected the output must be minimum positive");
        return 0;
    }
    else {
 ...
 * The result of this function is used to calculate the total time of components staying in the system for verification purpose.
 * \param[in] internal_process_time is an array of 5 element: the internal process time of inspector 1, inspector 2, workstation 1,workstation 2,workstation 3
 * \param[in] c1_in_buffer is an array of 3 element: number of components 1 in buffer 1, buffer 2, buffer 3
 * \param[in] c2_in_buffer2 is the number of components 2 in buffer 2
 * \param[in] c3_in_buffer3 is the number of components 3 in buffer 3
 * \param[in] comp_type_kept_by_insp2 is the type of component kept by inspector 2
 * \return num_of_c_in_sys is the number of components of each in the system.
 */
int *counters(float *internal_process_time,int *c1_in_buffer, int c2_in_buffer2,int c3_in_buffer3,int comp_type_kept_by_insp2){
    ...
    if ((internal_process_time==NULL) ||  (c1_in_buffer==NULL)){
        puts("Error in counters function: one of input doesn't exist ");
        puts("->The function expected the inputs exist");
        return NULL;
    }
    if ((c1_in_buffer[0]<0) || (c1_in_buffer[1]<0) || (c1_in_buffer[2]<0) || (c2_in_buffer2<0) || (c3_in_buffer3<0) || (comp_type_kept_by_insp2<2)||(comp_type_kept_by_insp2>3)){
        puts("Error in counters function: input with irrelevant values ");
        puts("->The function expected a number of component in buffer must be 0,1 or 2, and component type must be 1 or 2 ");
        return NULL;
    }
    ...
    {
        puts("Error in counters function: output with irrelevant values ");
        puts("->The function expected number of component in system non-negative and less or equal simulation length ");
        return NULL;
    }
    else {
...
/** \brief Function to decide the position of component 1  \n
 * This function processes the component 1 from inspector 1 and it sends it to the right workstation depending on the type of model
 * \param[in] c1_in_buffer is an array of 5 elements: the internal process time of inspector 1, inspector 2, workstation 1,workstation 2,workstation 3
 * \param[in] c2_in_buffer2 is the number of components 2 in Buffer 2.
 * \param[in] c3_in_buffer3 is the number of components 3 in Buffer 3.
 * \param[in] model_type is the type of model.
 * \return a buffer with the number of components 1 sent to each Buffer
 */
int *buffer1(int *c1_in_buffer,int c2_in_buffer2, int c3_in_buffer3, int model_type){
    ...
    if (c1_in_buffer==NULL) {
        puts("Error in buffer1 function: one of input doesn' exist ");
        puts("->The function expected input must exists\n");
        return NULL;
    }
    if ((c1_in_buffer[0]<0)||(c1_in_buffer[1]<0)||(c1_in_buffer[2]<0)||(c2_in_buffer2<0)||(c3_in_buffer3<0)){
        puts("Error in buffer1 function: input with irrelevant values ");
        puts("->The function expected that the value of the buffer was 0,1 or 2");
        return NULL;
    }
    ...
    if ((model_type<0) ||(model_type>2)){
        puts("Error in buffer1 function: input with irrelevant values ");
        puts("->The function expected that the model_type was 1 or 2");
        return NULL;
    }
    ...
    if ((buffer[0]<0)||(buffer[1]<0)||(buffer[2]<0)){
        puts("Error in buffer1 function: output with irrelevant values ");
        puts("->The function expected number of component in buffer must be 1 or 2");
        return NULL;
    }
...
/** \brief Function that model the flow in the facility and saves the states of the process. \n
 * The simulation states consists of: the simulation time, the number of components used and not used for the simulation, the number of products assembled,
 ...
 * the function proceed the input and configuration data and produce the result through the pointer my_state.
 * \param[in] my_config is the configuration that the simulator uses to run
 * \param[in] my_ptime is a pointer to array where the processing times are saved
 * \param[in] my_state is the states of the simulator
 * \return 0 if the functions run successfully, -1 if there is a fault in clockfcn function, -2 if there is a fault in counters function,
 *     -3 if there is a fault in buffer1 function, -4 if there is a fault in my_config input, -5 if there is a fault in my_ptime input.
 */
int work_flow(struct CONFIG *my_config,struct PTIME *my_ptime, struct STATE *my_state){
    ...
    if (my_config==NULL)
    {
        puts("Error in work_flow function: intput my_config doesnot exist");
        puts("->The function expected intput my_config must exist\n");
        return -1;
    }

    if ((my_config->sim_length<=0)||(my_config->sim_length>MAX_SIMULATION_LENGTH))
    {
        puts("Error in work_flow function: values of my_config->sim_length are irrelevant");
        printf("->The function expected (my_config->sim_length>0)||(my_config->sim_length<=%d)\n ", MAX_SIMULATION_LENGTH);
        return -1;
    }
    if ((my_config->both_models<0)||(my_config->both_models>1))
    {
        puts("Error in work_flow function: values of my_config->both_models are irrelevant");
        puts("->The function expected (my_config->both_models==0)||(my_config->both_models==1)");
        return -1;
    }
    if ((my_config->model_type<1)||(my_config->model_type>2))
    {
        puts("Error in work_flow function: values of model_type->model_type are irrelevant");
        puts("->The function expected (my_config->model_type==0)||(my_config->model_type==1)");
        return -1;
    }

    if (my_ptime==NULL)
    {
        puts("Error in work_flow function: my_ptime does not exist");
        puts("->The function expected my_ptime must exist");
        return -2;
    }
    for (int i=0;i<(my_config->sim_length);i++){
        ...
        puts("Error in work_flow function: one of the process time is non-positive");
        puts("->The function expected positive values from my_ptime");
        return -2;
        }
    }
    ...
    /* Event Ins2 can trigger: the internal process time of inspector 2 is 0 or negative and there is an available space in buffers for him to send C2/C3*/
        ...
        if ((internal_process_time[1] <= 0) && (type == 2 || type == 3) && (num_of_used_c2 <= (lengthc2-1)) ){ /* if the component the inspector 2 finishes is 2 or 3*/
            ...
            num_of_used_c3++;
            internal_process_time[1]=*(pointer_proc_time_c3+num_of_used_c3); /*loading input from file*/
            }
