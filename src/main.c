...
int main(int argc, char* argv[]) {
    ...
    read_config(config_name,my_config);             /* Call for the read_config*/
    ...
    while(actual_arg<argc) {        /*While there is an argument*/
        if(*argv[actual_arg]=='-') {/*If the argument starts with -*/
            if(strcmp(argv[actual_arg],"-v")==0) {  /*If the argument is -v*/
                ...
                /* Restart the values from the my_config structure */
                my_config->both_models=3;
                my_config->model_type=0;
                my_config->read_generate='\0';
                my_config->upper_limit=0;
                my_config->lower_limit=0;
                my_config->sim_length=0;
                /* The user does not provide for a model type equal 1 or 2 */
                while ((my_config->both_models != 0) && (my_config->both_models != 1)){
                    /* Print the instructions to the user */
                    puts("Please enter if you want to simulate a model or both models:\n");
                    puts("0: No\n");
                    puts("1: Yes\n");
                    scanf("%d",&my_config->both_models);/* Save the input into the configuration */
                if ((my_config->both_models != 0) && (my_config->both_models != 1)) { /* If the input is not valid */
                    puts("please type 0 or 1\n");   /* Show error message */
                }
                }
                if(my_config->both_models==0){
                 /* The user does not provide for a model type equal 1 or 2 */
                while ((my_config->model_type != 1) && (my_config->model_type != 2)){
                    /* Print the instructions to the user */
                    puts("Please enter the choice of model you want for the simulation:\n");
                    puts("1: The current/old model\n");
                    puts("2: The proposed model\n");
                    scanf("%d",&my_config->model_type);/* Save the input into the configuration */
                    if ((my_config->model_type != 1) && (my_config->model_type != 2)) { /* If the input is not valid */
                        puts("please type 1 or 2\n");   /* Show error message */
                    }
                    }
                }else{
                    my_config->model_type=2;
                }
                /* The user does not provide for a read or generate equal to r or g */
                while ((my_config->read_generate != 'r') && (my_config->read_generate != 'g')){
                    /* Print the instructions to the user */
                    ...
                    scanf(" %c",&my_config->read_generate);/* Save the input into the configuration */
                    if ((my_config->read_generate != 'r') && (my_config->read_generate != 'g')) { /* If the input is not valid */
                        puts("please type r or g\n");   /* Show error message */
                    }
                    if (my_config->read_generate=='g'){    /* If the option is g for generate*/
                        /* While the user does not provide for an upper limit bigger than the lower limit */
                        while (my_config->upper_limit <= my_config->lower_limit) {
                            /* Print the instructions to the user */
                            printf("please insert the lower limit of process time: ");
                            scanf("%f",&my_config->lower_limit); /* Save the input into the configuration */
                            printf("please insert the upper limit of process time: ");
                            scanf("%f",&my_config->upper_limit); /* Save the input into the configuration */
                            if(my_config->upper_limit <= my_config->lower_limit){ /* If upper limit is not bigger than lower limit */
                        ...
                        while ((my_config->sim_length < 1) || (my_config->sim_length > 5000)){
                            /* Print the instructions to the user */
                            printf("please enter the number of components of each type for simulation:");
                            scanf(" %d",&my_config->sim_length);/* Save the input into the configuration */
                            if(my_config->sim_length < 1){     /* If the simulation length is not bigger than 0*/
                                puts("please chose a simulation length bigger than 0"); /* Show error message */
                            }
                        }
                    }else{ /* If the user decides to read the files */
                        my_config->sim_length=MAX_SIMULATION_LENGTH; /* The simulation length equal to the constant */
                    }
                }
    ...
    return run_simulator(my_config, input_directory, output_directory);/* Call the function to run the simulator */
}
